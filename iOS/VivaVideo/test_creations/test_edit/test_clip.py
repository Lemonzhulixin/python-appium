...
class TestEditClip(TestCase):
    ...
    def test_clip_edit_01_video(self):
        ...
        try:
            ...
            actions.long_press(el_left_trim, None, None, 500).move_to(None, 300, None, 1500).release().perform()
            sc.capture_screen(fun_name, self.img_path)
            actions.long_press()
            ...
            actions = TouchAction(sc.driver)
            actions.long_press(el_right_trim, None, None, 500).move_to(None, 500, None, 1500).release().perform()
            sc.capture_screen(fun_name, self.img_path)
        ...
        try:
            sc.driver.find_element_by_name("倒放镜头").click()
            try:
                WebDriverWait(sc.driver, 120).until(
                    lambda reverse: reverse.find_element_by_name("倒放镜头"))
            except TimeoutError as t:
                sc.logger.error('倒放镜头超时', t)
                return False
            except Exception as e:
                sc.logger.error('倒放镜头出错了', e)
                return False
        except NoSuchElementException:
    ...
    def test_clip_edit_03_other(self):
        ...
        actions = TouchAction(sc.driver)
        actions.long_press(el_clip_01,None,None,1000).move_to(el_clip_02,None,None,1000).release().perform()
        time.sleep(1)
        sc.capture_screen(fun_name, self.img_path)
