...
class TestUserVideos(object):
    ...
    def test_video_origin(self):
        ...
        time.sleep(1)
        sc.logger.info('点击用户中心主按钮')
        btn_home = 'com.quvideo.xiaoying:id/img_studio'
        el_home = sc.driver.find_element_by_id(btn_home)
        el_home.click()
        time.sleep(1)
        el_video_title = 'com.quvideo.xiaoying:id/xiaoying_com_video_card_title'
        el_video = sc.driver.find_element_by_id(el_video_title)
    ...
    def test_video_download(self):
        ...
        time.sleep(1)
        sc.capture_screen(fun_name, self.img_path)
        btn_down = 'com.quvideo.xiaoying:id/download_btn'
        sc.driver.find_element_by_id(btn_down).click()
        sc.capture_screen(fun_name, self.img_path)

        try:
            sc.driver.find_element_by_id('com.quvideo.xiaoying:id/buttonDefaultPositive')
